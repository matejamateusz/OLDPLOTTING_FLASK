...
if '-help' in sys.argv:
    ...
    if arg2 == '-f' or arg2 == '-r':
        ...
        if str(arg3) not in myShelve.keys() or (dpPatterns[0],"values") not in myShelve[str(arg3)].keys():
        ...
        arg3 = 'interval'
        graph_arg2 = (start, stop, dpPatterns)
        if arg3 not in myShelve.keys() or (start, stop, dpPatterns[0], "values") not in myShelve[arg3].keys():
            print "Data not cached yet"
    ...
    for dp in dpPatterns:
        if arg2 == '-i-':
            mk_json(str(arg3), 'datahistogram'+arg2+str(i)+'.json', myShelve, dp, start, stop)
        else:
            mk_json(str(arg3), 'datahistogram'+arg2+str(i)+'.json', myShelve, dp)
        i += 1
